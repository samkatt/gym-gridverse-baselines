...
planning::
    ...
    python plan.py yaml/gv_empty.8x8.yaml mcts yaml/online_mcts.yaml
...
config files by appending any call with overwriting values, for example::
    ...
    python plan.py yaml/gv_crossing.5x5.yaml po-uct yaml/online_pouct.yaml num_sims=128
"""
...
def main():
    ...
    if args.observability == "mcts":
        rewards, _ = mdp_planning(env, create_mcts(env, **conf))
    if args.observability == "po-uct":
        rewards, _ = pomdp_planning(
            env,
